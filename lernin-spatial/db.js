...
export function getDB() {
  ...
  dbPromise = openDB(DB_NAME, DB_VERSION, {
    upgrade(db, oldVersion, newVersion, tx) {
      ...
      // --- Spatial learning (v8): landmarks, study paths, annotations
      if (oldVersion < 8) {
        if (!db.objectStoreNames.contains('landmarks')) {
          const lm = db.createObjectStore('landmarks', { keyPath: 'id' });
          lm.createIndex('by_deckId', 'deckId');
        }
        if (!db.objectStoreNames.contains('studyPaths')) {
          const sp = db.createObjectStore('studyPaths', { keyPath: 'id' });
          sp.createIndex('by_deckId', 'deckId');
        }
        if (!db.objectStoreNames.contains('annotations')) {
          const an = db.createObjectStore('annotations', { keyPath: 'id' });
          an.createIndex('by_deckId', 'deckId');
        }
      }
...
/** Alias used by the spatial map (same as saveConceptPosition). */
export async function saveConceptPositionOverride(cardId, pos) {
  return saveConceptPosition(cardId, pos.x, pos.y);
}

export async function clearConceptPosition(cardId) {
  const db = await getDB();
  return db.delete('conceptLayouts', cardId);
}
...
export async function saveLandmark(landmark) {
  const db = await getDB();
  const rec = {
    id: landmark.id || cryptoId(),
    deckId: landmark.deckId,
  ...
  await db.put('landmarks', rec);
  return rec;
}

export async function getLandmarksForDeck(deckId) {
  const db = await getDB();
  return db.getAllFromIndex('landmarks', 'by_deckId', deckId);
}

export async function deleteLandmark(id) {
  const db = await getDB();
  return db.delete('landmarks', id);
}

/* ---- Study Paths ---- */
export async function saveStudyPath(path) {
  const db = await getDB();
  const rec = {
    id: path.id || cryptoId(),
    deckId: path.deckId || null,
    name: path.name || 'Untitled path',
    nodeIds: Array.isArray(path.nodeIds) ? path.nodeIds : [],
    createdAt: path.createdAt || Date.now(),
    updatedAt: Date.now()
  };
  await db.put('studyPaths', rec);
  return rec;
}

export async function getStudyPathsForDeck(deckId) {
  const db = await getDB();
  if (deckId) return db.getAllFromIndex('studyPaths', 'by_deckId', deckId);
  return db.getAll('studyPaths');
}

export async function deleteStudyPath(id) {
  const db = await getDB();
  return db.delete('studyPaths', id);
}

/* ---- Annotations (text labels + freehand paths) ---- */
export async function saveAnnotation(ann) {
  const db = await getDB();
  const rec = {
    id: ann.id || cryptoId(),
    deckId: ann.deckId,
    type: ann.type || 'text', // 'text' | 'path'
    text: ann.text || null,
    x: ann.x ?? 0,
    y: ann.y ?? 0,
    pathData: ann.pathData || null, // SVG path d-string for freehand
...
export async function getAnnotationsForDeck(deckId) {
  const db = await getDB();
  return db.getAllFromIndex('annotations', 'by_deckId', deckId);
}
